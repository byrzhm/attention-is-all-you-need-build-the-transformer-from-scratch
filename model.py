...
def build_token_to_id_vocab(sentences, specials=("<pad>", "<bos>", "<eos>", "<unk>")):
    ...
    return {token: i for i, token in enumerate(dict.fromkeys(
        list(specials) + [word for sentence in sentences for word in sentence.split(" ")]
    ))}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    return [token_to_id.get(token, token_to_id[unk_token]) for token in sentence.split(" ")] if sentence else []
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    if ids_len > max_len:
        return ids[:max_len]
    ids.extend([pad_id] * (max_len - ids_len))
    return ids
...
def stack_padded_sequences_to_batch(padded_sequences):
    ...
    return torch.as_tensor(padded_sequences, dtype=torch.int64)
